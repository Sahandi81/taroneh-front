...
async function login(req, res) {
    if (req.method === "POST") {
        ...
        const backendRes = await fetch(`${_config_index__WEBPACK_IMPORTED_MODULE_1__/* .API_URL */ .T}/api/login`, {
            method: "POST",
            headers: {
        ...
        if (backendRes.ok) {
            // Set cookie
            await res.setHeader("Set-Cookie", cookie__WEBPACK_IMPORTED_MODULE_0___default().serialize("token", data.token, {
                httpOnly: true,
